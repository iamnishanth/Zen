...
for (let i = 1; i < 11; i++) {
  pagination += `<a href="#" class="a-tags pagination" onclick="displayProfile(this);">${i}</a>`;
}

div.innerHTML = pagination;
document.body.append(div);

const displayProfile = (selectedElement) => {
  let index = selectedElement.innerText;
  let start = (Number(index) - 1) * 10 + 1;
  let end = Number(index + "0");

  let table = "<table><tbody>";

  for (let i = start - 1; i <= end - 1; i++) {
    table += `<tr><td>${JSON_DATA[i].id}</td><td>${JSON_DATA[i].name}</td><td>${JSON_DATA[i].email}</td></tr>`;
  }

  table += "</tbody></table>";

  document.querySelector(".profile").innerHTML = table;

  // Switch Active Page CSS
